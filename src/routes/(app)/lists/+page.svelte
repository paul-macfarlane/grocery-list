...
<ul id="lists">
    {#each data.groceryLists as groceryList}
        <li class="list-card">
            <div>
                {groceryList.title} - {groceryList.items.length} item{groceryList.items.length !== 1 ? `s` : `` }
            </div>
            ...
            <div class="list-card-bottom">
                ...
                <div class="list-card-buttons">
                    <IconLinkButton imageClass="icon" alt="edit list" href={`/lists/${groceryList.id}`} src={editSvg}/>

                    <form method="POST" action={`/lists/${groceryList.id}?/delete`}>
                        <IconButton imageClass="icon" type="submit" alt="delete list" src={deleteSvg}/>
                    </form>
                </div>
